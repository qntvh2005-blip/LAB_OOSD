:ON ERROR EXIT
-- Bật Query > SQLCMD Mode trước khi chạy. Không xóa dữ liệu cũ.
IF DB_ID(N'QuanLyKhachSan_Test') IS NULL CREATE DATABASE QuanLyKhachSan_Test;
GO
USE QuanLyKhachSan_Test;
GO

IF OBJECT_ID('dbo.NhanVien','U') IS NOT NULL
 THROW 50000, N'Database đã có bảng. Dừng để bảo vệ dữ liệu; hãy dùng database mới.', 1;
GO
SET XACT_ABORT ON;
BEGIN TRANSACTION;
...
CREATE UNIQUE INDEX UQ_NguoiLT_Phieu_CCCD ON NguoiLuuTru(SoPhieuDat,SoCMND);
COMMIT TRANSACTION;
GO
